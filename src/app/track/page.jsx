...
export default function TrackPage() {
  ...
  const handleCheck = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch(`${SERVER}/api/orders/track?orderId=${encodeURIComponent(orderId)}&phone=${encodeURIComponent(phone)}`);
    ...
    <main className="bg-[var(--color-paper)] min-h-screen">
      <div className="max-w-[480px] mx-auto px-4 pb-16">
        ...
        <div className="mt-6 space-y-4">
          <div>
            <label className="font-mono text-[11px] tracking-[0.08em] uppercase text-[var(--color-olive)] mb-1.5 block">Order ID</label>
            <input
          ...
          <div>
            <label className="font-mono text-[11px] tracking-[0.08em] uppercase text-[var(--color-olive)] mb-1.5 block">WhatsApp number</label>
            <input
        ...
        {result && (
          <div className="mt-5 border-[1.5px] border-[var(--color-ink)] bg-[var(--color-kraft)] p-4">
            ...
            <div className="mb-3">
              <span className="inline-block px-3 py-1 text-[13px] font-bold text-white"
            ...
            {result.status !== "Cancelled" && (
              <div className="flex border-[1.5px] border-[var(--color-ink)] overflow-hidden">
                {STATUS_STEPS.map((step, i) => (
                  <div key={step} className={`flex-1 text-center py-2 px-1 text-[10px] font-mono leading-tight border-r border-[var(--color-ink)] last:border-r-0 transition-colors ${
                    i < currentIdx ? "bg-[var(--color-ink)] text-white" :
                    i === currentIdx ? "bg-[var(--color-rust)] text-white" : "bg-white text-[var(--color-olive)]"
                  }`}>
                    {step.replace(" ", "\n")}
                  </div>
                ))}
        ...
        <a href="/" className="inline-block mt-6 text-[13.5px] text-[var(--color-rust)] font-semibold hover:underline">
          ← Place a new order
        </a>
